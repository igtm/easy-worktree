...
class ReleaseScriptTests(unittest.TestCase):
    ...
    def test_resolve_version_prefers_local_pyproject(self):
        with mock.patch.dict(sys.modules, {"toml": mock.MagicMock()}):
            sys.modules.pop("easy_worktree", None)
            module = importlib.import_module("easy_worktree")
            try:
                with mock.patch("easy_worktree.metadata.version", return_value="9.9.9"):
                    self.assertEqual(module._resolve_version(), "0.2.13")
            finally:
                sys.modules.pop("easy_worktree", None)
